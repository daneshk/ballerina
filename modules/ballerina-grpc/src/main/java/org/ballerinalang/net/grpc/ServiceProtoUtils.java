...
public class ServiceProtoUtils {
    ...
    private static Message generateMessageDefinition(BType messageType) {
        ...
        for (BStructType.StructField structField : ((BStructType) messageType).getStructFields()) {
            Field messageField;
            Annotation fieldAnnotation = structField.getAnnotation("ballerina.net.grpc", "protobuf");
            AnnAttrValue nameAnnValue = fieldAnnotation != null ? fieldAnnotation.getAnnAttrValue("name") : null;
            String fieldName = nameAnnValue != null ? nameAnnValue.getStringValue() : structField.getFieldName();
            AnnAttrValue indexAnnValue = fieldAnnotation != null ? fieldAnnotation.getAnnAttrValue("index") : null;
            fieldIndex = indexAnnValue != null ? (int) indexAnnValue.getIntValue() : ++fieldIndex;
            if (structField.getFieldType() instanceof BArrayType) {
                BType fieldType = ((BArrayType) structField.getFieldType()).getElementType();
                AnnAttrValue typeAnnValue = fieldAnnotation != null ?
                        fieldAnnotation.getAnnAttrValue("fieldType") : null;
                String sfieldtype = typeAnnValue != null ? typeAnnValue.getStringValue() : fieldType.getName();
                messageField = Field.newBuilder(fieldName).setLabel("repeated").setType
                        (sfieldtype).setIndex(fieldIndex).build();
            } else {
                AnnAttrValue typeAnnValue = fieldAnnotation != null ?
                        fieldAnnotation.getAnnAttrValue("fieldType") : null;
                String sfieldtype = typeAnnValue != null ?
                        typeAnnValue.getStringValue() : structField.getFieldType().getName();
